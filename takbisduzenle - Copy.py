...
class TapuProcessor:
    ...
    def update_headers(self):
        ...
        try:
            ...
            # İşlem yapılan kayıtları X ile işaretle
            cursor.execute("""
                UPDATE tapu_verileri 
                SET hucreno_11 = 'X'
                WHERE hucreno_11 IS NULL OR hucreno_11 != 'X'
    ...
    def assign_tasinmaz_numbers(self):        
        ...
        cursor = conn.cursor()        
        try:
            # Her bir tapu kaydının başlangıç ve bitiş satırlarını belirle
            cursor.execute("""
                WITH TapuBolum AS (
                    SELECT 
                        rowid,
    ...
    def delete_marked_rows(self):
        """'SILINECEK' olarak işaretlenmiş satırları siler"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    
        try:
            # Silinecek satır sayısını al
            cursor.execute("SELECT COUNT(*) FROM tapu_verileri WHERE baslikontrol = 'SILINECEK'")
            count = cursor.fetchone()[0]
        
            # Silme işlemini gerçekleştir
            cursor.execute("DELETE FROM tapu_verileri WHERE baslikontrol = 'SILINECEK'")
        
            conn.commit()
            #print(f"Toplam {count} adet işaretli satır silindi")
    ...
    def update_tapu_kayit_bilgisi(self):
        ...
        try:
            ...
            conn.commit()
            #print(f"Toplam {updates} kayıt güncellendi")
            return True
