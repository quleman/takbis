...
class IpotekKoordinatExtractor:
    ...
    def is_ipotek_page(self, page):
        """Sayfanın ipotek veya boş rehin sayfası olup olmadığını kontrol eder."""
        try:
            text = page.extract_text()
        
            # Debug için
            #print("Sayfa içeriği:", text)
        
            # Şerh veya haciz sayfası kontrolü
            if "Şerh" in text or "Kamu Haczi" in text:
                return False
            
            # İpotek veya boş rehin başlığını kontrol et
            if not ("Ipotek" in text or "BosRehin" in text or 'KanuniRehin' in text or 'IpotekliBorcSenedi' in text or "İpoteğin Konulduğu Hisse Bilgisi" in text ):
                return False
        
            # PDF'deki temel başlıkları kontrol et
            temel_basliklar = [
                "Alacaklı",
                "Borç", 
                "Borçlu Malik",
            ...
            # Tüm temel başlıkların var olup olmadığını kontrol et
            baslik_sayisi = 0
            for baslik in temel_basliklar:
                if baslik in text:
                    baslik_sayisi += 1
                
            # En az 2 temel başlık varsa ipotek sayfasıdır
            return baslik_sayisi >= 2
